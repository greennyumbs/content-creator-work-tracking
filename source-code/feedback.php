...
<body id="page-top">
                        ...
                        <div  style="margin-left: auto; margin-right: auto;width: 25rem;">
                            ...
                            <div class="card shadow ">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Select subsidiary and role</h6>
                                </div>
                                <div class="card-body">
                                    <form action="feedbacktab.php" method="POST">
                                        <div class="form-row">
                                            <div class="form-group col-md-6">
                                                <label for="subsidiary">Subsidiary</label>
                                                <select id="subsidiary" name="subsidiary" class="form-control" onchange="Checkstat()">
                                                    <option value="" selected></option>
                                                    <?php
                                                        $result = mysqli_query($con,"SELECT DISTINCT subsidiary_name FROM subsidiary");
                                                        while($row = mysqli_fetch_array($result)){
                                                            echo "<option value=";
                                                            echo '"'.$row['subsidiary_name'].'"';
                                                            echo ">".$row['subsidiary_name']."</option>";
                                                        }
                                                    ?>
                                                </select>
                                            </div>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-6">
                                                ...
                                                <select id="role" name="role" class="form-control" onchange="Checkstat()">
                                                    <option value="" selected></option>
                                                    <?php
                                                            $result = mysqli_query($con,"SELECT DISTINCT role_name FROM role");
                                                            while($row = mysqli_fetch_array($result)){
                                                                echo "<option value=";
                                                                echo '"'.$row['role_name'].'"';
                                                                echo ">".$row['role_name']."</option>";
                                                            }
                                                    ?>
                                                </select>
                                            </div>
    ...
    <script>
        function Checkstat()
        {
            const sub = document.getElementById('subsidiary').value
