...
<div id="content" style=" margin-top: 25px">
    ...
    <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Least number of videos.</h1>
        <p class="mb-4">
                        Least number of videos in each subsidiaries by team.
                    </p>

        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Least number of videos.</h6>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            ...
                            <?php
                                $query = "SELECT sd.subsidiary_name, t.team_name, COUNT( DISTINCT v.video_id ) AS video_count
                                            FROM subsidiary sd
                                            INNER JOIN staff s ON sd.subsidiary_id = s.subsidiary_id
                                            INNER JOIN team t ON s.team_id = t.team_id
                                            INNER JOIN video v ON t.team_id = v.team_id
                                            GROUP BY sd.subsidiary_name, t.team_name
                                            HAVING COUNT( DISTINCT v.video_id) = (
                                                SELECT MIN(video_count)
                                                FROM (
                                                    SELECT sd2.subsidiary_name, t2.team_name, COUNT( DISTINCT v2.video_id) AS video_count
                                                    FROM Subsidiary sd2
                                                    INNER JOIN Staff s2 ON sd2.subsidiary_id = s2.subsidiary_id
                                                    INNER JOIN Team t2 ON s2.team_id = t2.team_id
                                                    INNER JOIN Video v2 ON t2.team_id = v2.team_id
                                                    GROUP BY sd2.subsidiary_name, t2.team_name
                                                ) AS counts
                                                WHERE counts.subsidiary_name = sd.subsidiary_name
                                                GROUP BY counts.subsidiary_name);";
