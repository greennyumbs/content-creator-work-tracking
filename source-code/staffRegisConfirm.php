                            ...
                            <div class="card shadow">
                                ...
                                <div class="card-body">
                                    ...
                                    if(empty($_POST['firstName'])){
                                        echo "Please Input data Staff First Name" ;
                                    }elseif(empty($_POST['lastName'])){
                                        echo "Please Input data Staff Last Name" ;
                                    }elseif(empty($_POST['role'])){
                                        echo "Please Input data Role ID" ;
                                    }elseif(empty($_POST['team'])){
                                        echo "Please Input data Team ID" ;
                                    }elseif(empty($_POST['subsidiary'])){
                                        echo "Please Input data Sub ID" ;
                                    }else{

                                    // $team = mysqli_real_escape_string($connection, $_POST['team']);

                                    // $sql_temp = mysqli_fetch_array(mysqli_query($connection,"SELECT team_id FROM team WHERE team_name = '$team'"));
                                    
                                    // $team_id_temp = $sql_temp['team_id'];
                                    
                                    // if($sql_temp['team_id'] === NULL)
                                    // {
                                    //     $sql_insert = "INSERT INTO team (team_name)
                                    //                     VALUES ('$team_id_temp')";
                                    
                                    //     $team =  mysqli_fetch_array(mysqli_query($connection,"SELECT COUNT(*) FROM team")) + 1;
                                    // }
                                    // else{
                                    //     $team = $sql_temp;
                                    // }
                                    ...
                                    $query = "SELECT team_id FROM team WHERE team_name = '$team'";
                                    $result = mysqli_query($connection, $query);
                                    $row = mysqli_fetch_assoc($result);

                                    if ($row) {
                                    // Team already exists, retrieve the team_id
                                    $team_id = $row['team_id'];
                                    } else {
                                    // Team does not exist, insert it into the 'team' table
                                    $sql_insert = "INSERT INTO team (team_name) VALUES ('$team')";
                                    ...
                                    $team_id = mysqli_insert_id($connection);
                                        ...
                                        // $team = mysqli_real_escape_string($connection, $_POST['team']);
                                        $sub = mysqli_real_escape_string($connection, $_POST['subsidiary']);
                                    
                                        $sql="INSERT INTO Staff (staff_fname, staff_lname, role_id, team_id, subsidiary_id)
                                        VALUES ('$firstname', '$lastname', '$role', '$team_id', '$sub')";
                                        if (!mysqli_query($connection,$sql)) {
                                            die('Error: ' . mysqli_error($connection));
                                        }
                                        if(!empty($_POST['middleName'])){
                                            $middlename = mysqli_real_escape_string($connection, $_POST['middleName']);
                                            $staff = mysqli_fetch_array(mysqli_query($connection,"SELECT staff_id FROM staff WHERE staff_fname = '$firstname' AND staff_lname = '$lastname'"));
                                            $staffID = $staff['staff_id'];
                                            if (!mysqli_query($connection, "UPDATE staff SET staff_mname = '$middlename' WHERE staff_id = '$staffID'")) {
