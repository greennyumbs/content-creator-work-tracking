...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <?php
            ...
            $subID = mysqli_real_escape_string($con, $_POST['SubID']);
            $subname = mysqli_real_escape_string($con, $_POST['SubName']);

            // SQL statement to insert data into Guest and Guest_history tables
            $sql = "INSERT INTO subsidiary (subsidiary_id, subsidiary_name) VALUES ('$subID', '$subname') " ;
            
            if (!mysqli_multi_query($con, $sql)) {
                die('Error: ' . mysqli_error($con));
            }

            if(!empty($_POST['SubMail'])){
                $submail = mysqli_real_escape_string($con, $_POST['SubMail']);
                if (!mysqli_query($con, "UPDATE subsidiary SET subsidiary_email = '$submail' WHERE subsidiary_id = '$subID'")) {
                    die('Error: ' . mysqli_error($con));
                }
            }
            if(!empty($_POST['SubFloor'])){
                $subfloor = mysqli_real_escape_string($con, $_POST['SubFloor']);
                if (!mysqli_query($con, "UPDATE subsidiary SET subsidiary_floor = '$subfloor' WHERE subsidiary_id = '$subID'")) {
                    die('Error: ' . mysqli_error($con));
                }
            }
            if(!empty($_POST['SubPhone'])){
                $subphone = mysqli_real_escape_string($con, $_POST['SubPhone']);
                if (!mysqli_query($con, "UPDATE subsidiary SET subsidiary_phone = '$subphone' WHERE subsidiary_id = '$subID'")) {
                    die('Error: ' . mysqli_error($con));
                }
            }
                    ...
                    <div class="row">
                        ...
                        <div  style="margin-left: auto; margin-right: auto; margin-top: auto; margin-bottom: auto; width: 25rem;">
                            ...
                            <div class="card shadow ">
                                ...
                                <div class="card-body">
                                    <form name="sub_regis" method="post" action="index.php">
